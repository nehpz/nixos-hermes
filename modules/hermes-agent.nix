...

{
  ...
  # SOUL.md must live in $HERMES_HOME, not workingDirectory. The `documents` option
  # installs to workingDirectory (workspace/) but prompt_builder.py reads exclusively
  # from get_hermes_home()/SOUL.md. This script runs after hermes-agent-setup has
  # created the .hermes directory and set ownership, so the install is safe.
